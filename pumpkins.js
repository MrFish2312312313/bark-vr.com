...
(function () {
  ...
  function isCut(prepped, pu, pv) {
    if (!prepped.tape) {
      for (const { st, uu } of prepped)
        if (st.tool === KNIFE && strokeHits(st, uu, pu, pv)) return true;
      return false;
    }
    let cut = false;
    for (const { st, uu } of prepped) {
      if (st.tool === KNIFE) { if (!cut && strokeHits(st, uu, pu, pv)) cut = true; }
      else if (st.tool === TAPE) { if (cut && strokeHits(st, uu, pu, pv)) cut = false; }
    }
    return cut;
  }
  ...
  function scrapeDepth(prepped, pu, pv) {
    ...
    for (const { st, uu } of prepped) {
      if (st.tool === SCRAPER) {
        if (strokeHits(st, uu, pu, pv))
          best = Math.max(best, Math.min(1, Math.max(0, st.depth == null ? 0.6 : st.depth)));
      } else if (st.tool === TAPE && best > 0 && strokeHits(st, uu, pu, pv)) {
